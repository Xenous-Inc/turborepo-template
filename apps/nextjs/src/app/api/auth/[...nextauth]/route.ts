...
import { handlers, isSecureContext } from '@xenous/auth';

export const runtime = 'edge';

const EXPO_COOKIE_NAME = '__xenous-expo-redirect-state';
...
export const GET = async (_req: NextRequest, props: { params: { nextauth: string[] } }) => {
    ...
    if (nextauthAction === 'callback' && !!isExpoCallback) {
        ...
        if (!match)
            throw new Error(`Unable to find session cookie: ${JSON.stringify(authResponse.headers.getSetCookie())}`);
